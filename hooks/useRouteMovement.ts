...
import { deskPointByAgent, instructionRouteByAgent, officeRoutes, routePoints } from "@/data/officeRoutes";
...
export function useRouteMovement(step: DemoStep | null, demoStatus: DemoStatus, speed: DemoSpeed, instructionStates: AgentAnimationMap, manualRoute: ManualRoute = null) {
  const plans = useMemo(() => Object.fromEntries(agentIds.map(id => {
    ...
    return [id, step?.routes[id]];
  })), [instructionStates, manualRoute, step]);
  ...
  useEffect(() => {
    ...
    if (demoStatus === "paused" || (!step && !hasInstruction && !manualRoute)) return;
  ...
  }, [demoStatus, instructionStates, manualRoute, plans, speed, step]);
  ...
  return useMemo(() => Object.fromEntries(agentIds.map(id => {
    ...
    const work = instructionStates[id] ?? step?.animations[id] ?? "idle";
  ...
  })), [indices, instructionStates, plans, step]);
}
