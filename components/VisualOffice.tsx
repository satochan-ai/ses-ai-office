...
export default function VisualOffice() {
  ...
  const demo = useOfficeDemo(); const agentAnimation = useAgentAnimation(); const motions = useRouteMovement(demo.step?.id, demo.status, demo.speed, agentAnimation.states, manualRoute);
  ...
  return <div className={`${s.officePage} ${demo.step ? s.demoRunning : ""}`}><OfficeHeader onGuide={() => setGuideOpen(true)} /><main className={s.officeMain}><div className={s.statusStrip}>{demoBadges.map((badge, index) => <span key={badge}><i className={s[`stripDot${index}`]} />{badge}</span>)}</div><div className={s.officeIntro}><div><span className={s.eyebrow}><Building2 size={13} />TWO-FLOOR LIVE WORKSPACE</span><h1>11名のAI社員が、2つのフロアで連携しています</h1><p>営業・人材運用から顧客深耕、契約、教育までを一つのAIオフィスで確認できます。</p></div>{demo.status === "idle" ? <DemoStart onStart={demo.begin} speed={demo.speed} onSpeed={demo.setSpeed} /> : <span className={s.updated}><Clock3 size={13} />18:30 更新</span>}</div><FloorSwitch activeFloor={activeFloor} onChange={setActiveFloor} /><div className={s.unifiedWorkspace}><OfficeFloor floorId={activeFloor} agents={visibleAgents} motions={motions} activeIds={demo.step?.agentIds ?? []} matchingProgress={demo.matchingProgress} handoffLabel={demo.step && activeFloor === "1f" ? handoffLabels[demo.step.id] : undefined} onSelect={selectAgent} /><div className={s.workspaceRail}><ActivityRail /><AgentDirectory agents={visibleAgents} activeFloor={activeFloor} onSelect={selectAgent} /></div></div><div className={s.mobileHint}><Sparkles size={14} />フロアを切り替え、AI社員をタップすると仕事と成果を確認できます</div></main><div className={s.srOnly} aria-live="polite">{floorAnnouncement}</div>{selected && <AgentDetailPanel agent={selected} speech={demo.step?.speeches[selected.id] ?? selectedSpeech} history={selectedHistory} collaborationOptions={collaborationRoutesByAgent[selected.id] ?? []} onClose={() => setSelected(null)} onRun={runInstruction} onCollaborate={routeId => runCollaboration(selected, routeId)} />}{demo.step && demo.status !== "idle" && <DemoControls status={demo.status} step={demo.step} progress={demo.progress} matchingProgress={demo.matchingProgress} onPause={demo.pause} onResume={demo.resume} onStop={demo.stop} onRestart={demo.begin} />}{demo.resultOpen && <DemoResult logs={demo.logs} onClose={() => demo.setResultOpen(false)} onRestart={demo.begin} />}{guideOpen && <FirstVisitGuide open={guideOpen} onClose={closeGuide} />}{toast && <Toast message={toast} />}</div>;
}
