...
function OfficeRoom({ agent, onSelect, active, matchingProgress }: { agent: OfficeAgent; onSelect: (agent: OfficeAgent) => void; active: boolean; matchingProgress: number }) {
  const speechFocus = agent.status === "要対応" || agent.status === "分析中";
  return <button className={`${s.room} ${s[`room_${agent.accent}`]} ${agent.id === "manager" ? s.managerRoom : ""} ${active ? s.demoRoomActive : ""}`} onClick={() => onSelect(agent)} aria-label={`${agent.room}・${agent.name}の詳細を開く`}><div className={s.roomHeader}><div><span className={s.roomNumber}>{agent.floor === "upper" ? "2F" : "1F"}</span><strong>{agent.room}</strong></div><AgentStatus status={agent.status} /></div><AgentSpeechBubble text={agent.speech} focus={speechFocus || active} /><div className={s.roomScene}><RoomDecoration type={agent.decoration} /><DeskSetup type={agent.decoration} />{agent.id === "matching" && matchingProgress > 0 && <span className={s.demoMatchBadge}>照合中 {matchingProgress}%</span>}<div className={s.avatarSpot}><span className={s.chair} /><AIAgentAvatar agent={agent} /><span className={s.agentName}>{agent.name}</span></div></div><div className={s.roomFooter}><strong>{agent.currentTask}</strong><div><span>{agent.metrics[0]}</span></div></div><span className={s.roomOpen}>詳細を見る <ChevronRight size={12} /></span></button>;
}

function OfficeFloor({ floor, agents, onSelect, activeIds, matchingProgress }: { floor: "upper" | "lower"; agents: OfficeAgent[]; onSelect: (agent: OfficeAgent) => void; activeIds: string[]; matchingProgress: number }) {
