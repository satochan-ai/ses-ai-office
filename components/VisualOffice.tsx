...
import { AgentWorkAnimation } from "./AgentWorkAnimation";
import { WorkHandoff } from "./WorkHandoff";
...
function AIAgentAvatar({ agent, large = false, animation = "idle" }: { agent: OfficeAgent; large?: boolean; animation?: string }) {
  ...
  return <div className={`${s.robot} ${s[`accent_${agent.accent}`]} ${s[`persona_${agent.decoration}`]} ${s[`robot_${animation}`]} ${large ? s.robotLarge : ""}`} aria-hidden="true"><div className={s.antenna}><i /></div><div className={s.robotHead}><div className={s.face}><span /><span /><b /></div></div><div className={s.robotBody}><i className={s.collar} /><b>{agent.shortName}</b><Accessory size={11} /></div><span className={s.armLeft} /><span className={s.armRight} /><span className={s.legLeft} /><span className={s.legRight} /><CharacterProps type={agent.decoration} /><span className={s.roleTag}>{roleLabels[agent.decoration]}</span></div>;
}
...
function OfficeRoom({ agent, onSelect, active, matchingProgress, animation }: { agent: OfficeAgent; onSelect: (agent: OfficeAgent) => void; active: boolean; matchingProgress: number; animation: ReturnType<typeof animationForAgent> }) {
  const speechFocus = agent.status === "要対応" || agent.status === "分析中";
  return <button className={`${s.room} ${s[`room_${agent.accent}`]} ${agent.id === "manager" ? s.managerRoom : ""} ${active ? s.demoRoomActive : ""} ${animation !== "idle" ? s.roomWorking : ""}`} onClick={() => onSelect(agent)} aria-label={`${agent.room}・${agent.name}の詳細を開く`}><div className={s.roomHeader}><div><span className={s.roomNumber}>{agent.floor === "upper" ? "2F" : "1F"}</span><strong>{agent.room}</strong></div><AgentStatus status={agent.status} /></div><AgentSpeechBubble text={agent.speech} focus={speechFocus || active} /><div className={s.roomScene}><RoomDecoration type={agent.decoration} /><DeskSetup type={agent.decoration} /><AgentWorkAnimation agent={agent} state={animation} matchingProgress={matchingProgress} />{agent.id === "matching" && matchingProgress > 0 && <span className={s.demoMatchBadge}>照合中 {matchingProgress}%</span>}<div className={s.avatarSpot}><span className={s.chair} /><AIAgentAvatar agent={agent} animation={animation} /><span className={s.agentName}>{agent.name}</span></div></div><div className={s.roomFooter}><strong>{agent.currentTask}</strong><div><span>{agent.metrics[0]}</span></div></div><span className={s.roomOpen}>詳細を見る <ChevronRight size={12} /></span></button>;
}

function OfficeFloor({ floor, agents, onSelect, activeIds, matchingProgress, animations }: { floor: "upper" | "lower"; agents: OfficeAgent[]; onSelect: (agent: OfficeAgent) => void; activeIds: string[]; matchingProgress: number; animations: Record<string, ReturnType<typeof animationForAgent>> }) {
  return <section className={`${s.floor} ${floor === "upper" ? s.upperFloor : s.lowerFloor}`} aria-label={floor === "upper" ? "2階" : "1階"}><div className={s.floorLabel}>{floor === "upper" ? "2F  STRATEGY" : "1F  OPERATIONS"}</div><div className={s.floorRooms}>{agents.map(agent => <OfficeRoom key={agent.id} agent={agent} onSelect={onSelect} active={activeIds.includes(agent.id)} matchingProgress={matchingProgress} animation={animations[agent.id]} />)}</div></section>;
}

function OfficeBuilding({ agents, onSelect, activeIds, matchingProgress, animations, step }: { agents: OfficeAgent[]; onSelect: (agent: OfficeAgent) => void; activeIds: string[]; matchingProgress: number; animations: Record<string, ReturnType<typeof animationForAgent>>; step: ReturnType<typeof useOfficeDemo>["step"] }) {
  return <div className={s.buildingWrap}><div className={s.roof}><span>SES AI OFFICE</span><div className={s.roofLight} /></div><div className={s.building}><OfficeFloor floor="upper" agents={agents.filter(a => a.floor === "upper")} onSelect={onSelect} activeIds={activeIds} matchingProgress={matchingProgress} animations={animations} /><OfficeFloor floor="lower" agents={agents.filter(a => a.floor === "lower")} onSelect={onSelect} activeIds={activeIds} matchingProgress={matchingProgress} animations={animations} /></div><WorkHandoff step={step} /><div className={s.foundation}><span /><span /><span /></div></div>;
}
...
export default function VisualOffice() {
  ...
  const agentAnimation = useAgentAnimation();
  ...
  const animations = Object.fromEntries(officeAgents.map(agent => [agent.id, animationForAgent(agent, demo.step?.id, Boolean(agentAnimation.states[agent.id]), agentAnimation.states[agent.id] === "completed")])) as Record<string, ReturnType<typeof animationForAgent>>;
