...
export default function OfficeFloor({ agents, motions, activeIds, matchingProgress, handoffLabel, onSelect }: { agents: OfficeAgent[]; motions: Record<string, RoutedAgentMotion>; activeIds: string[]; matchingProgress: number; handoffLabel?: string; onSelect: (agent: OfficeAgent) => void }) {
  ...
  return <section className={s.floor} aria-label="SES AI Office フロア">
    <div className={s.windows}><i /><i /><i /><i /></div><OfficePath />
    ...
    {agentLayouts.map(layout => { const agent = agents.find(item => item.id === layout.agentId); return agent ? <HumanAgent key={agent.id} agent={agent} layout={layout} motion={motions[agent.id]} active={activeIds.includes(agent.id)} beingVisited={visitedIds.has(agent.id)} onSelect={onSelect} /> : null; })}
